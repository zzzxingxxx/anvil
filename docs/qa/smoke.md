...
2. 当前工作区信任在对话顶栏切换（未信任 / 已信任）。设置页保存 bash 白名单，保存后有成功提示。已打开工作区时写入项目 `.anvil/settings.json`；下次打开该仓库会覆盖全局，设置页表单会跟着刷新。可添加多套 OpenAI 兼容 URL + Key（可起名），顶栏按接口分组切换模型。密钥只写 `~/.pi/agent/models.json`。底栏出错时显示「出错」而不是「就绪」。看板空列有说明。
