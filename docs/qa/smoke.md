...
2. 设置页保存 bash 白名单，保存后有成功提示。已打开工作区时写入项目 `.anvil/settings.json`；下次打开该仓库会覆盖全局，设置页表单会跟着刷新。有探测到的模型时默认模型用下拉选择，点外面关闭。可填 OpenAI 兼容 URL + Key 拉取模型（密钥只写 `~/.pi/agent/models.json`）。底栏出错时显示「出错」而不是「就绪」。看板空列有说明。
