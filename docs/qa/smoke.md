...
2. 左侧粘贴本仓库路径，状态栏显示 cwd。若该仓库已有官方 Pi jsonl，假循环/SDK/RPC 都会恢复最近一次会话。
...
1. 信任后委派 2 个只读子任务，托盘同时更新，显示耗时；完成后出现横幅，不打断输入框。假循环/RPC 子任务走 Fake；SDK 父会话子任务走独立 SdkPiAdapter，审批卡带「来自子任务」。
2. 子任务不能再委派。
3. 两个实现者抢同一文件，第二个失败。
