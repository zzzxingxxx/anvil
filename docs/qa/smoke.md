...
2. 左侧粘贴本仓库路径，状态栏显示 cwd。若该仓库已有官方 Pi jsonl，假循环/SDK/RPC 都会恢复最近一次会话。换另一个目录后，旧子任务托盘、Diff 快照和用量会清空。SDK 模式顶栏显示 SDK 徽章。
3. 未信任发「列出文件」：bash/write 被拒。
4. 对话顶栏切到已信任 → 假循环弹出审批 → 允许一次有输出；拒绝则工具失败、Host 不崩。
5. 新建两条会话，切换后消息和用量不串。运行中改信任或切模型会提示「等当前轮结束」。Ctrl+K 忙时只列出 `/abort`。
6. 刷新浏览器：snapshot 把对话和工具卡推回来。已结束的工具不再转圈，已用时还在。
7. 运行中按 Esc：立刻停；若已点「插入方向 / 结束后做」，文本还回输入框。不中止时，假循环会把插入方向写进本轮回复，结束后自动开下一轮「结束后做」。闲时点这两项（含 SDK/RPC）会立刻开一轮。再发一条新 prompt 会提示「等当前轮结束」。审批卡显示倒计时，超时视为拒绝，超时后不能再点允许。Composer `/` 忙时只列出 `/abort`。运行中打开另一个工作区、点最近工作区、选文件夹、切会话或分叉会提示「等当前轮结束」。设置页忙时只读。子任务完成横幅约 8 秒后自动收起。复制失败会进错误横幅。命令超过 30 秒无响应会提示请求超时。
8. `pnpm check && pnpm test` 绿。
9. **需密钥：** 真模型改一个 md；官方 `pi` resume 同一 jsonl。

当前本机：已装 `pi`，`~/.pi/agent` 存在。`pnpm --filter host compat` 用 SessionManager 打开了 5 个真实 jsonl（version 3）。2026-09-18 用本机 OpenAI 兼容自定义模型跑通第 9 步：Anvil SDK 写出 `docs/anvil-dogfood.md`，官方 `pi --session` 回复 `RESUME_OK`。分叉会话回复 `FORK_OK`。SDK `architect` 子任务 `succeeded`。

## Phase 2

1. 假会话里能看到树节点，单击历史节点后刷新会只显示该路径消息。右键可 Fork / 从这里继续 / 复制节点 id。运行中切换分支会提示「等当前轮结束」。
2. 对同一用户消息 Fork 两次，两条分支互不覆盖。
3. 假循环结束后 Diff 面板出现 `.anvil/demo-diff.txt`；点还原走 `artifact.restore`（不会改 README），该路径从 Diff 消失。新增文件点「删除新增」会删掉该文件；被删文件点「恢复文件」会写回。有 git 时优先显示 `git diff`。刷新浏览器会清掉旧错误横幅。运行中保存设置、侧栏新建、检查器压缩会禁用。
...
2. 当前工作区信任在对话顶栏切换（未信任 / 已信任）；未打开工作区时按钮禁用。设置页可滚动。保存 bash 白名单后有成功提示。已打开工作区时写入项目 `.anvil/settings.json`；下次打开该仓库会覆盖全局，设置页表单会跟着刷新。可添加多套 OpenAI 兼容 URL + Key（可起名）。添加后立刻出现在顶栏和设置页模型列表，点选即切到当前模型。密钥只写 `~/.pi/agent/models.json`。底栏出错时显示「出错」而不是「就绪」。看板空列有说明。
