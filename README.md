...
最近项目记在 `~/.anvil/config.json`。默认角色写在 `~/.anvil/personas/`（architect / implementer / reviewer.md），已改过的文件不会被覆盖。打开工作区会合并项目 `.anvil/settings.json`（覆盖 bash 白名单 / 默认模型），并恢复该仓库最近一次官方 Pi 会话（没有才新建）。默认 **未信任**：禁止 bash / write / edit。对话顶栏点「未信任」切到「已信任」后，bash 仍会弹出「允许一次 / 拒绝」。RPC sidecar 不走 Anvil 闸门。

## 真 Pi 与假循环

默认使用 `@earendil-works/pi-coding-agent@0.85.1`。没有配置模型时，顶栏会显示「选择模型」。设置页可添加多套 OpenAI 兼容 URL + Key（写入本机 `~/.pi/agent/models.json`，不进仓库）。添加后立刻出现在顶栏和设置页，点选即切换当前模型。
