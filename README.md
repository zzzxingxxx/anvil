...
最近项目记在 `~/.anvil/config.json`。默认角色写在 `~/.anvil/personas/`（architect / implementer / reviewer.md），已改过的文件不会被覆盖。打开工作区会合并项目 `.anvil/settings.json`（覆盖 bash 白名单 / 默认模型）。默认 **未信任**：禁止 bash / write / edit。点顶栏「沙箱保护」切换为信任后，bash 仍会弹出「允许一次 / 拒绝」。
