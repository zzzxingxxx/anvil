...
describe("FakePiAdapter", () => {
  ...
  it("rejects newSession while a turn is running", async () => {
    const state = createWorkspaceState();
    state.trust = "untrusted";
    const adapter = new FakePiAdapter(state);
    const prompt = adapter.prompt({ text: "还在跑" });
    await expect(adapter.newSession("忙时新建")).rejects.toThrow(/等当前轮结束/);
    await adapter.abort();
    await prompt;
  });
  ...
  it("rejects compact while a turn is running", async () => {
    ...
    const adapter = new FakePiAdapter(state);
    const prompt = adapter.prompt({ text: "还在跑" });
    await expect(adapter.compact("保留目标")).rejects.toThrow(/等当前轮结束/);
    await adapter.abort();
    await prompt;
  });
